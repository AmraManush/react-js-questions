...
There is no free API that returns live flight prices, so this project generates **mock flight offers on top of real airport data**: the distance between two airports is calculated for real using their latitude/longitude, and that real distance is used to scale believable fake prices, durations, and stop counts.
...
- Random airline (pick from a small fixed list)
- Random stop count (0, 1, or 2)
- Duration based on distance ÷ average speed, plus layover time if there are stops
- Price based on distance, with more stops costing slightly less and nonstop costing slightly more
...
Each flight card must show stop count (`Nonstop`, `1 stop`, `2 stops`) and the real distance in km, alongside price and duration.
